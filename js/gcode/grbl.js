// GRBL Post-Prozessor.
PM.registerPost({
  id: 'grbl', name: 'GRBL', ext: 'nc',
  emit(tp, opts = {}) {
    const feed = opts.feed ?? 800;
    const plungeFeed = opts.plungeFeed ?? Math.round(feed / 3);
    const spindle = opts.spindle ?? 12000;
    const f3 = PM.f3, safeZ = tp.safeZ;
    const L = [];
    L.push('; PatternMaster — GRBL Export');
    L.push('; Nullpunkt (X0 Y0): links-unten. Z0 = Werkstueckoberflaeche, Z- = Material.');
    L.push('; Einheiten mm, Absolutkoordinaten.');
    L.push('G21 G90 G17 G94');
    if (spindle) L.push('M3 S' + spindle);
    L.push('G0 Z' + f3(safeZ));
    let prev = null;
    for (const m of tp.moves) {
      if (m.type === 'rapid') {
        L.push(`G0 X${f3(m.x)} Y${f3(m.y)} Z${f3(m.z)}`);
      } else {
        const isPlunge = prev && Math.abs(m.x - prev.x) < 1e-4 && Math.abs(m.y - prev.y) < 1e-4 && m.z < prev.z - 1e-4;
        L.push(`G1 X${f3(m.x)} Y${f3(m.y)} Z${f3(m.z)} F${isPlunge ? plungeFeed : feed}`);
      }
      prev = m;
    }
    L.push('G0 Z' + f3(safeZ));
    if (spindle) L.push('M5');
    L.push('M2');
    return L.join('\n');
  }
});
